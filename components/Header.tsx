    ...
    >
      <div className="container mx-auto px-4 lg:px-8">
        <div className="flex items-center justify-between relative">
          ...
          >
            <span className="text-3xl font-black text-white tracking-tighter uppercase">KIA</span>
            <div className="h-6 w-px bg-gray-600 mx-1"></div>
            <span className="text-lg font-bold text-white tracking-widest uppercase whitespace-nowrap">
              {CONTACT_INFO.name}
            </span>
          </div>
          ...
          <nav className="hidden lg:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 items-center space-x-8 font-medium text-sm text-gray-300">
            {navItems.map((item) => (
              <a 
                ...
                className={`hover:text-white transition-colors uppercase tracking-wide text-xs font-bold ${
                  (currentView === item.view && !item.hash) ? 'text-kia-red' : ''
                }`}
              >
                {item.label}
              </a>
